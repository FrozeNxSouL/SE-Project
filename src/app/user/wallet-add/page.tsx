    ...
    <div className="flex justify-center items-center w-full">
      <div className="flex flex-col gap-4">
        ...
        <label className="input input-bordered flex items-center gap-2 justify-center">
          ...
          <h1><span className="font-bold">Wallet</span> {cashInWallet} ฿</h1>
        </label>
        <label className="input input-bordered flex items-center gap-2 justify-center">
          Top-up amount
          <input
            type="number" // Change input type to "number"
            className="grow text-end bg-transparent"
            step={0.5}
            onChange={(e) => setTopUpAmount(parseInt(e.target.value))}
          />
        </label>
        <button className="btn btn-warning " onClick={handleTopUp}>TOP UP</button>
      </div>
    </div>
  );
}
